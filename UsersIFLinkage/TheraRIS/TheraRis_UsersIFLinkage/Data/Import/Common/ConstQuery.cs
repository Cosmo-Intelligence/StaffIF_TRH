...
namespace TheraRis_UsersIFLinkage.Data.Import.Common
{
    ...
    class THERARIS_QUERY
    {
        ...
        /// クライアントユーザ管理 Merge
        /// </summary>
        public const string RTRIS_USERMANAGECOMP_MERGE =
                ...
                //+ "     COMMISSION = {3},"
                //+ "     COMMISSION2 = {4},"
                //+ "     VIEWRACCESSCTRLFLAG = {5},"
                //+ "     VIEWCACCESSCTRLFLAG = {6}"
                + " when not matched then"
                + "   insert"
                + "     (USERID,"
                + "      HOSPITALID,"
    ...
    public const string RRIS_SECTIONDOCTORMASTER_MERGE =
                  " merge into RRIS.SECTIONDOCTORMASTER"
                + " using("
...
// Y_Higuchi -- del --
                ...
                //+ "     USEFLAG = {8},"
                //+ "     PHYSICIST_FLG = {9},"
                //+ "     PLAN_DECISION_AUTHORITY_FLG = {10},"
                //+ "     DEFAULT_MENU_GROUP = {11}"
                + " when not matched then"
                + "   insert"
                + "     (DOCTOR_ID,"
                + "      DOCTOR_NAME,"
...
// Y_Higuchi -- del --
		...
		public const string RRIS_SECTIONDOCTORMASTER_DELETE =
                  " update RRIS.SECTIONDOCTORMASTER"
                + " set"
                + "   USEFLAG = {1}"
                + " where"
                + "   DOCTOR_ID = {0}";
        ...
        public const string RRIS_SECTIONDOCTORMASTER_SELECT_SHOWORDER =
            ...
            + "  end as SHOWORDER"
            + "  from"
            + "  (select max(SHOWORDER) as SHOWORDER from RRIS.SECTIONDOCTORMASTER WHERE SHOWORDER BETWEEN {0} AND {1}) a,"
            + "  (select max(SHOWORDER) as SHOWORDER from RRIS.SECTIONDOCTORMASTER WHERE SHOWORDER >= {2}) b)";

        /// <summary>
        /// 診療科医師マスタ 表示順取得
        /// </summary>
        public const string RRIS_SECTIONDOCTORMASTER_SELECT_MAX_SHOWORDER =
              " (select nvl(max(SHOWORDER), 0) + 1 as SHOWORDER from RRIS.SECTIONDOCTORMASTER)";

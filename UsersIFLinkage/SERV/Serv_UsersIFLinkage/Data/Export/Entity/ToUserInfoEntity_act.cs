...
namespace Serv_UsersIFLinkage.Data.Export.Entity
{
	class ToUserInfoEntity_act : ToUsersInfoEntity
  {
    string[] flds = new string[] { F_REQUESTID, F_REQUESTDATE, F_DB, F_APPCODE
      ...
      , F_TRANSFERTEXT };
    private const string TABLE_NAME="TOUSERINFO_SRV";
    ...
    public ToUserInfoEntity_act()
    {
      ...
      foreach (string val in flds)
      {
        ...
        {
          // MESSAGEID1 は 投入条件名(EC01/EC02/EC04)を入れる
    ...
    {
      string strSQL;
      strSQL = "UPDATE " + TABLE_NAME + " SET " + F_MESSAGEID1 + " = '" + SYORI_TAISYO_MSG1 + "' WHERE (" + F_TRANSFERSTATUS + " = '" + SYORI_TAISYO_STATUS + "') AND (" + F_MESSAGEID1 + " IS NULL)";
      return strSQL;
    }
    ...
    /// <summary>
    /// MESSAGEID1="{1}" を "{0}" に変更
    /// </summary>
    /// <returns></returns>
    public string Get_CONDUPDSQL()
    {
      string strSQL;
      strSQL = "UPDATE " + TABLE_NAME + " SET " + F_MESSAGEID1 + " = '{0}' WHERE (" + F_TRANSFERSTATUS + " = '" + SYORI_TAISYO_STATUS + "') AND (" + F_MESSAGEID1 + " = '{1}' )";
      return strSQL;
    }
